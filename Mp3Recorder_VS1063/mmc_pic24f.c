...
/* Se debe definir los pines SDCARD_CS Y SDCARD_CD*/

#define CS_LOW()    SDCARD_CS = 0	/* MMC CS = L */
#define CS_HIGH()   SDCARD_CS = 1	/* MMC CS = H */

// Comentar si no se va ausar el Card Detect
// #define SDCARD_USE_CD
...
int wait_ready(void) {
    ...
    do
        d = xchg_spi(0xFF); while ((d != 0xFF) && Timer2);
...
BYTE send_cmd(
    ...
    do
        res = xchg_spi(0xFF); while ((res & 0x80) && --n);
