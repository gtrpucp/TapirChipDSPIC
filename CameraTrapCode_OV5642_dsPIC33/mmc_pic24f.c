...
#define CS_LOW()    SDCARD_CS = 0	/* MMC CS = L */
#define CS_HIGH()   SDCARD_CS = 1	/* MMC CS = H */

//#define CD	(!_RB11)	/* Card detected   (yes:true, no:false, default:true) */
//#define CD	(!SDCARD_CD)	/* Card detected   (yes:true, no:false, default:true) */
...
#define	SPI_BUS_SET_TO_FULL_SPEED_SD	SPI1STATbits.SPIEN = 0; SPI1CON1bits.PPRE1 = 1; SPI1CON1bits.PPRE0 = 1; SPI1CON1bits.SPRE2 = 1; SPI1CON1bits.SPRE1 = 0; SPI1CON1bits.SPRE0 = 1; SPI1STATbits.SPIEN = 1;

#define	FCLK_SLOW   SPI_BUS_SET_TO_LOW_SPEED      /* Set slow clock (100k-400k) */
#define	FCLK_FAST   SPI_BUS_SET_TO_FULL_SPEED_SD  /* Set fast clock (depends on the CSD) */
...
extern volatile uint8_t dma0_TxIntFlag;
...
void power_on (void)
{
    ...
    //SPI1CON1bits.SPRE = 0b111; //secondary prescal 1:1
    //SPI1CON1bits.PPRE = 0b01; //primary prescal 16:1
...
static
void xmit_dma_multi (const BYTE * buff)
{
    dma0_TxIntFlag = 0;
    
    DMA_ForceTransfer(buff);
    
    while(dma0_TxIntFlag == 0) {
        Nop();
    }
...
)
{
	do {
		...
		//SPI1BUF = *buff++; while(!_SPIRBF) ; SPI1BUF; 
        IFS0bits.SPI1IF = 0;//check
        SPI1BUF = *buff++; while(!IFS0bits.SPI1IF) ; SPI1BUF; 
        IFS0bits.SPI1IF = 0;//check
		SPI1BUF = *buff++; while(!IFS0bits.SPI1IF) ; SPI1BUF; 
	} while (cnt -= 2);
}
...
)
{
	do {
		...
		//SPI1BUF = 0xFF; while(!_SPIRBF) ; *buff++ = SPI1BUF; 
        IFS0bits.SPI1IF = 0;//check
		SPI1BUF = 0xFF; while(!IFS0bits.SPI1IF) ; *buff++ = SPI1BUF; 
        IFS0bits.SPI1IF = 0;//check
		SPI1BUF = 0xFF; while(!IFS0bits.SPI1IF) ; *buff++ = SPI1BUF; 
	} while (cnt -= 2);
}
...
)
{
	...
	if (token != 0xFD) {	/* Not StopTran token */        
        ...
        if (useDma) {
            ...
            do{
                resp = xchg_spi(0xFF);
                //toma 3 intentos para retonar un valor distinto de 0xFF
                tries++;
            }while(resp == 0xFF);
...
)
{
    ...
    //deselect();
	for (n = 10; n; n--){
        if(!xchg_spi(0xFF))	/* 80 dummy clocks */
            return STA_NOINIT;
    }
...
//#ifdef PLACA_CAMARA_PUCP
//    //Sino, asumimos que la sd está presente
//	s &= ~STA_NODISK;    
//#else
//	if (CD) s &= ~STA_NODISK;
