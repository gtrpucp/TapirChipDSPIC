...
int counter=2;
char answer[7]={0};
char nuevafoto[12]= "N100001.JPG", fotoanterior[12]="N000000.JPG";
...
void loop() {
  ...
  if(counter-->0){
    ...
    if(trx.send_httpcon("reg")){
        if(trx.send_str("NODO2",5)){
          DBG_OUTPUT_PORT.println("\nSTRING NODO 2 SENT OK\n");  
          while(1){
  ...
  if(trx.rec_buf(answer)){  /* In case something is received  */
    ...
    if(!strcmp(answer,"SLEEP_")){    /*In case server sends node to SLEEP*/         
      ...
      func.Sleep();//--------------------------------------------------------------------
        ...
        while(strcmp((char*)nuevafoto,(char*)fotoanterior)){
